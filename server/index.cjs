  ...
  try {
    ...
    const approve = db.transaction(() => {
      ...
      if (se) {
        ...
        db.prepare('INSERT INTO student_events (student_id, event_id, status, points_collected) VALUES (?, ?, ?, 1)')
          .run(submission.student_id, submission.event_id, submission.claim_type === 'won' ? 'won' : 'participated');
      }
    });
